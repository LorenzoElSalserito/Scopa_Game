...
class MazzoGiocatore(object):
    ...
    def scope (self, carte_mano, carte_tavolo)
        for carte in carte_tavolo:
			if carte [valore] == carte_tavolo [valore] and carte in carte_tavolo == 1:
    ...
    def calcola_punteggio(self):
        ...
        for carte in self.carte
            if len(lista_carte) > 20:
				punteggio = punteggio+1
				print ('Hai fatto un punto, il numero di carte che hai è:' len(lista_carte))
...
class GiocoCarteNapolatane(object):
    def __init__(self, giocatori, 
        ...
        if (len(giocatori)%2): raise Exception('numero giocatori incompatibile')    
    ...
    def inizia(self):
        if not self.tavolo and self.numero_carte_tavolo_inizio:
            ...
            raise Exception('Specificare il numero delle carte da giocare sul tavolo, es: 4')
...
if __name__ == '__main__':
    ...
    print("Carte sul tavolo: {}".format( ' '.join([str(i) for i in gioco.tavolo.carte])))
